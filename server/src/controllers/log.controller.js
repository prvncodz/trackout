  ...
  try {
    ...
    if (log[0]?.exercises?.sets?.length) {
      try {
        ...
        allsets.forEach(
          async (set) => await Set.findByIdAndDelete(set, { session }).lean(),
        );
      } catch (err) {
...
const DuplicateLog = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const { _id, __v, completedAt, createdAt, updatedAt, exercises, ...copy } =
      log;
    ...
    await Promise.all(
      exercises.map(async (e) => {
        ...
        const { _id, __v, createdAt, updatedAt, sets, ...exerciseCopy } =
          exercise;
        ...
        await Promise.all(
          sets.map(async (s) => {
            ...
            const { _id, __v, createdAt, updatedAt, completed, ...setCopy } =
              set;
    ...
    return res
      .status(201)
      .json(
        new ApiResponse(201, dupelog[0], "duplicate log created successfully"),
      );
  } catch (error) {
    await session.abortTransaction();
    throw error;
