...
const SignUpUser = asyncHandler(async (req, res) => {
    try {
        ...
        const userExists = await User.find({ fullname });
        if (userExists) {
            throw new ApiError(500, "user with this fullname already exists")
        }
        ...
        throw new ApiError(500, "User registeration failed", error.name, error);
    }
})
