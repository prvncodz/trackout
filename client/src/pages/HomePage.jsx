      ...
      <Dialog open={editing} onOpenChange={setEditing}>
        ...
        <DialogContent className="sm:max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription>Make changes to the log. Click save when you&apos;re done.</DialogDescription>
          </DialogHeader>
  ...
  return (
    <div className="border-line-color relative flex h-screen w-full shrink-0 flex-col items-end justify-start gap-3 overflow-auto border-r bg-neutral-50 px-5 py-10 lg:w-180">
      ...
      <div className="flex h-auto w-full flex-col items-center justify-center gap-2 lg:mt-15">
        {logs?.map((log) => (
          <Log key={log._id} log={log} ActiveLog={ActiveLog} setActiveLog={setActiveLog} setAllLogs={setAllLogs} />
        ))}
      </div>
    </div>
  )
                ...
                <Dialog open={isCreating} onOpenChange={setIsCreating}>
                  ...
                  <DialogContent className="sm:max-w-sm">
                    <DialogHeader>
                      ...
                      <DialogDescription>Create new set. Click save when you&apos;re done.</DialogDescription>
                    </DialogHeader>
                    <form onSubmit={(e) => handleCreateSet(e)}>
                      <FieldGroup>
                        ...
                        <Field>
                          ...
                          <Input id="reps" name="reps" type="number" min={0} step={1} defaultValue="" placeholder="0" />
                        </Field>
                ...
                <AlertDialog>
                  ...
                  <AlertDialogContent size="sm">
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription>This will permanently delete this exercise log.</AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                      <AlertDialogCancel variant="outline">Cancel</AlertDialogCancel>
            ...
            <div className="flex gap-3">
              ...
              {log?.exercises?.length > 0 && (
                <Button variant="outline" disabled={log?.completedAt} onClick={handleMarkLogCompleted}>
                  <IconCircleDashedCheck size={18} /> {log?.completedAt ? "Completed" : "Mark as Completed"}
                </Button>
              )}
            </div>
          </div>
...
const HomePageContent = () => {
  ...
  return (
    <div className="flex h-screen w-full flex-col items-start justify-start lg:flex-row">
      ...
      {ActiveLog && (
        <>
          <ShowLog logId={ActiveLog} isActive={ActiveLog !== null} ActiveLog={ActiveLog} setActiveLog={setActiveLog} />
        </>
      )}
    </div>
  )
