...
const DashboardMockup = () => {
    ...
    return (
        <div className="my-20 ml-15 flex h-150 w-300 cursor-default shadow-aesthetic items-center overflow-hidden rounded-lg select-none lg:ml-0 lg:w-auto">
            <div className={`border-line-color flex h-full w-60 shrink-0 flex-col border-r bg-white p-3 dark:bg-near-black dark:border-gray-800`}>
                <div className="flex-start mt-2 ml-0 flex w-full px-4">
                    <Logo className={"text-[13px] font-extrabold text-gray-800 antialiased"} size={"sm"} />
                </div>

                {/* Nav */}
                <nav className="space-y-0.9 mt-5 ml-1 flex-1 gap-1 px-3 pt-1">
                    {NAV_ITEMS.map((item) => {
                        ...
                        return (
                            <button
                                ...
                                className={`flex w-full cursor-pointer items-center gap-2 rounded-lg px-3 py-3 text-left text-xs font-medium transition-all duration-200 ${isActive
                ...
                <UserInfo avatarUrl={"/mock-user-pfp.jpeg"} fullName={"Bruce wayne"} size={"sm"} />
            </div>
