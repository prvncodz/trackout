...
const HamburgerButton = ({ className = "", ...props }) => {
    ...
    return (
        <button
            className={`flex h-10 w-10 cursor-pointer isolate items-center justify-center rounded-md bg-gray-100 p-2 font-semibold transition-all hover:bg-neutral-100 active:scale-98 duration-10 relative ${className}`}
            {...props}
            onClick={() => setIsNavOpen(!isNavOpen)}
        >
            <IconMenu2 size={20} />
            {isNavOpen && (
                <motion.nav className="space-y-0.8 isolate absolute z-10 top-0 right-10 mt-10 ml-1 w-65 flex-1 rounded-lg bg-gray-100 px-3 py-4 shadow-md transition-all"
                    initial={{
                        opacity: 0,
                    }}
                    animate={{
